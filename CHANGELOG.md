...
- **自动化部署脚本**：新增 `scripts/deploy.sh`，一键完成 Miniconda 安装、conda 环境创建、依赖安装、systemd 服务配置与启动，支持 `--env-file` 参数指定密钥文件。

### 变更

- **边界框渲染简化**：移除边界框左上角的标签文字与背景色块，渲染结果只保留彩色矩形框，减少视觉干扰。
- **坐标系修复**：`_extract_bbox` 新增对 Qwen-VL 原生 0-1000 归一化坐标的支持，修复了边界框与目标位置不对应的问题。
- `DETECTION_PROMPT_TEMPLATE` 明确要求模型以 0-1000 归一化格式返回坐标，与模型原生输出格式对齐。
